class ZTab {
  constructor(selector, options = {}) {
    ...
    this.tabs = Array.from(
      this.container.querySelectorAll(this.opt.tabSelector)
    );
  ...
  _init() {
    ...
    const savedTabId = params.get(this.paramKey);

    const initialTab =
      this.opt.remember && savedTabId
        ? this.tabs.find(
            (t) =>
    ...
    this._activateTab(initialTab, false, false);
  ...
  _activateTab(tab, triggerOnChange = true, updateURL = this.opt.remember) {
    this.tabs.forEach((t) =>
      t.closest("li").classList.remove(this.opt.activeClassName)
    );
    ...
    if (triggerOnChange && typeof this.opt.onChange === "function") {
      this.opt.onChange({ tab, panel: activePanel });
    }
  }

  destroy() {
    this.tabs.forEach((tab) =>
      tab.removeEventListener("click", this._handleClick)
    );
    this.tabs.forEach((t) =>
      t.closest("li").classList.remove(this.opt.activeClassName)
    );
    this.panels.forEach((p) => (p.hidden = false));
    this.container = null;
    this.tabs = null;
    this.panels = null;
  }
...
class ZPop {
  ...
  _build() {
    ...
    this._backdrop.className = "Zpop";

    const container = document.createElement("div");
    container.className = "Zpop__container";
    this.opt.cssClass.forEach((cls) => cls && container.classList.add(cls));

    // Nút đóng nhanh
    if (this.opt.closeMethods.includes("button")) {
      const closeBtn = this._createButton("&times;", "Zpop__close", () =>
        this.close()
      );
    ...
    this._modalContent.className = "Zpop__content";
    this._updateContent();
    ...
    if (this.opt.footer) {
      ...
      this._modalFooter.className = "Zpop__footer";
  ...
  _updateContent() {
    ...
    if (this.content) {
      ...
      this._modalContent.append(this.template.content.cloneNode(true));
    }
  }
  _renderFooter() {
    if (!this._modalFooter) return;
    this._modalFooter.innerHTML = this._footerContent || "";
    this._footerButtons.forEach((btn) => this._modalFooter.append(btn));
  }

  addFooterButton(title, cssClass, callback) {
    const button = this._createButton(title, cssClass, callback);
    this._footerButtons.push(button);
    if (this._modalFooter) this._renderFooter();
  }

  _onTransitionEnd(callback) {
    const onEnd = (e) => {
      if (["opacity", "transform", "visibility"].includes(e.propertyName)) {
        this._backdrop.removeEventListener("transitionend", onEnd);
        if (typeof callback === "function") callback();
      }
    };
    this._backdrop.addEventListener("transitionend", onEnd);
  }

  open() {
    if (!this._backdrop) this._build();

    ZPop.elements.push(this);
    this._backdrop.offsetHeight;
    this._backdrop.classList.add("Zpop--show");

    if (this.opt.enableScrollLock) this._lockScroll();

    if (this.opt.closeMethods.includes("overlay")) {
      this._backdrop.onclick = (e) => {
        if (e.target === this._backdrop) this.close();
      };
    }

    if (this.opt.closeMethods.includes("escape")) {
  ...
  _handleEscapeKey(e) {
    if (e.key === "Escape" && this.opt.closeMethods.includes("escape")) {
      this.close();
    }
  }
  _lockScroll() {
    const target = this.opt.scrollLockTarget();
    if (ZPop.elements.length === 1 && this._hasScrollbar(target)) {
      const scrollWidth = this._getScrollbarWidth();
      target.style.paddingRight = `${scrollWidth}px`;
      target.classList.add("Zpop--no-scroll");
    }
  }

  _unlockScroll() {
    const target = this.opt.scrollLockTarget();
    if (ZPop.elements.length === 0) {
      target.style.paddingRight = "";
      target.classList.remove("Zpop--no-scroll");
    }
  }
  ...
  close() {
    ...
    this._backdrop.classList.remove("Zpop--show");
  ...
  _createButton(text, cssClass, callback) {
    const btn = document.createElement("button");
    btn.innerHTML = text;
    if (cssClass) {
      cssClass.split(" ").forEach((cls) => btn.classList.add(cls));
    }
    btn.onclick = (e) => {
      e.stopPropagation();
...
class ZSlide {
  constructor(selector, options = {}) {
    this.container = document.querySelector(selector);
    if (!this.container) {
      console.error(`ZSlide: Container "${selector}" not found!`);
      return;
    }
  ...
  _init() {
    this.container.classList.add("Zslide-wrapper");
  ...
  _createContent() {
    ...
    this.content.className = "Zslide-content";
  ...
  _createTrack() {
    ...
    this.track.className = "Zslide-track";
    ...
    this.slides.forEach((slide) => {
      slide.classList.add("Zslide-slide");
  ...
  _createNav() {
    ...
    this.navWrapper.className = "Zslide-nav";
  ...
  moveSlide(step) {
    ...
    // Xử lý nhảy vị trí khi loop (vô hình với người dùng)
    if (this.opt.loop) {
      this.track.addEventListener(
        "transitionend",
        () => {
          const clones = this._getCloneCount();
      ...
      // Giới hạn cho slider không loop
      this.currentIndex = Math.max(
        0,
        Math.min(this.currentIndex, slideCount - this.opt.items)
      );
      this._updatePosition();
      setTimeout(() => {
        this._isAnimating = false;
  ...
  _updateNav() {
    ...
    if (this.opt.loop) {
      const clones = this._getCloneCount();
      activeDot = (this.currentIndex - clones + slideCount) % slideCount;
    } else {
      activeDot = this.currentIndex;
    }

    const dotIndex = Math.floor(
      activeDot / (this.opt.slideBy === "page" ? this.opt.items : 1)
    );
    Array.from(this.navWrapper.children).forEach((dot, i) => {
